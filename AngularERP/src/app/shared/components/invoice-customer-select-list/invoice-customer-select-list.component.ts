...
@Component({
  selector: 'app-invoice-customer-select-list',
  templateUrl: './invoice-customer-select-list.component.html',
  styleUrls: ['./invoice-customer-select-list.component.scss']
})
export class InvoiceCustomerSelectListComponent implements OnInit,OnChanges {
  @Input() modalName: any;
  ...
  @Output() onChange = new EventEmitter<InvoiceCustomerModel>();
  @Output() onSelectAddress = new EventEmitter<CustomerLocation>();
  @Input() elementId:string;
  compId: number;
  companyId: number;
  customers: InvoiceCustomerModel[] = [];
  filtered: any[] = [];
  ...
  getCustomer(type) {
    this.salePurchaseService.getInvoiceCustomer(this.companyId, type)
      .subscribe(response => {
        this.customers = response as InvoiceCustomerModel[];
        this.filtered = response as InvoiceCustomerModel[];
      }, err => {
        this.customers = [];
  ...
  onSelect(customer) {
    if (customer) {
      ...
      this.onChange.emit(new InvoiceCustomerModel());
    }
    ...
    this.accChartService.saveAccChart(this.newCustomerModel)
      .subscribe((response: any) => {
        if (response.status && response.newAccChart) {
          var newCustomer = new InvoiceCustomerModel();
          ...
          newCustomer.address = response.newAccChart.address;
